...
//! 两种模式都复用 ai-gateway 的 WorkBuddy 账号库，但保持独立的当前账号。
...
/// ai-gateway 的旧 helper，允许直接原地升级，并清理旧文件。
...
mod tests {
    ...
    fn helper_test_dir() -> PathBuf {
        std::env::temp_dir().join(format!("ai-gateway-codebuddy-helper-{}", uuid::Uuid::new_v4()))
    }
  ...
  "env": { "HTTPS_PROXY": "http://127.0.0.1:7890" }
    ...
    fn extracts_node_path_without_accepting_shell_noise() {
        // 夹具必须用当前平台的绝对路径：该函数靠 is_absolute() 过滤 shell 噪声，
        // 而 "/Users/..." 在 Windows 上缺盘符、不算绝对路径，整行会被丢弃。
        let node = std::env::temp_dir().join("node");
        let output = format!("welcome to the shell\n{}\n", node.display());
        assert_eq!(node_path_from_shell_output(output.as_bytes()), Some(node));
    ...
    fn helper_selects_active_account_and_fails_without_selected_token() {
        ...
        let output = Command::new("node")
            ...
            .env("AI_GATEWAY_ACCOUNTS_FILE", &accounts_file)
            .output()
            .unwrap();
