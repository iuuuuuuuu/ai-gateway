...
///
/// 原实现默认关闭，理由是「求解器在没有真人操作的情况下产出通过凭证，
/// 要不要用它应当由账号所有者决定，由用户在界面上明确开启」。
///
/// 那个理由有两个问题。其一，**界面上根本没有这个开关**：实测前端全目录搜
/// `captcha` / 「验证码」0 命中，宿主 `commands.rs` 同样 0 命中 ——
/// 注释承诺了一个不存在的入口，于是「由用户决定」实际是「谁也决定不了，
/// 功能永远关着」。其二，**不开就完全不能用**：实测真实对话请求，ZCode
/// 对话通道对不带验证码的请求一律回
/// `HTTP 400 {"code":3007,"msg":"captcha verify failed"}` ——
/// 即「默认关闭」等于 ZCode 对话**从未成功过**。
///
/// 所有者 2026-09-20 明确指示：
///
/// > 「肯定要默认打开并且不能关闭啊，这是开源软件有什么在乎的？」
///
/// 他的判断成立：求解本身是**在精简环境里跑阿里云官方 SDK**
///（`assets/zcode-captcha/solver.js` 用 happy-dom 模拟浏览器加载官方
/// 验证 SDK，走完正常流程），**不是逆向破解算法**；账号是他自己的；
/// 开源软件也没有"替他保守"的立场。
///
/// 故改为**始终启用**，不再读配置、不提供开关。
///
/// ⚠ 保留函数而不是内联 `true`：网关配置里仍会写
/// `zcode_captcha_enabled: true`（见 `write_gateway_config`），
/// 那个键现在只是**如实反映**实际行为，供诊断时对照 ——
/// 留着旧配置值为 false 也不会再关掉功能（**不做回退**，
/// 因为"关了就不能用"不是可接受的配置）。
...
fn write_native_config(cfg: &Value) -> Result<PathBuf, String> {
    ...
    let native = json!({
        ...
        "pool": {
            ...
            // 凭证目录**热加载**（新增账号免重启）。
            //
            // 为什么显式写出而不是靠 Go 侧缺省：本仓库的风格是
            // 「每个键都显式写出」，缺省值一旦在两边分叉就会出现
            // 「界面里加了账号、网关看不到」。显式写出来，两边一致
            // 这件事在代码里可见，不必去翻 Go 侧的 default。
            //
            // ⚠ 间隔 5 秒：够快（用户加完账号几乎立刻可见），
            // 又不至于频繁 stat 目录。指纹只算「文件名 + mtime + size」，
            // **不读文件内容**（那是秘密，且慢）。
            "watch_auth_dir": true,
            "watch_auth_dir_interval": "5s",
            ...
            // 网关无从推断这个路径，故由宿主告诉它。
            //
            // ⚠ `zcode_captcha_enabled` 现在**恒为 true**（2026-09-20 改）。
            //
            // 原先默认 false，理由是"要用户明确开启"。但那个界面开关
            // **从来不存在**，而 ZCode 对话通道对不带验证码的请求一律回
            // `HTTP 400 {"code":3007}` ⇒ "默认关闭"等于**对话从未成功过**。
            //
            // 所有者原话：「肯定要默认打开并且不能关闭啊，这是开源软件
            // 有什么在乎的？」。求解是跑阿里云官方 SDK（非逆向破解），
            // 账号属于用户，开源软件没有替他保守的立场。
            //
            // 这个键现在只是**如实反映**实际行为，供诊断时对照；
...
/// 「指定账号」下拉里的单个账号条目。
///
/// `note` 必须一并下发：界面上的账号名按 **备注 → 昵称 → uid 前缀** 取，
/// 而备注是用户自己在「账号管理」里填的标签（如「公司号」），恰恰是分辨
/// 「这是谁的号」的唯一可靠线索 —— 上游昵称对国服账号常为空，uid 又是一串随机串。
/// 此前只下发 nickname，于是**设了备注的账号在下拉里依旧显示 uid 前缀**，
/// 等于备注白设。
///
/// 缺 uid 的脏记录直接跳过：SelectItem 的 value 不能为空串，
/// 否则 Radix 会抛错并让整页白屏。
///
/// 与 `account_meta` 的 needsRelogin 同口径：复用 `refresh::needs_relogin`，
/// 不直接读原始标记（旧版本把网络抖动也写成了 needs_relogin）。
/// 前端手动模式的「可选账号」过滤依赖这个字段，误报会让用户看到
/// 「明明能用的号却选不了」。
...
/// 而**模式与放行名单**的变更不会因改配置而变化（网关启动时读它们）。
/// 于是用户切到手动模式后，池里仍是全部账号，
/// 必须手动点「重启」才真正生效（这正是「切换模式要重启」的根因）。
///
/// ⚠ 注意区分两类变更（2026-09-19 更新）：
///
///	· **账号凭证的新增/删除** —— 现在**不需要重启**了：
///	  Go 侧有 `pool.watch_auth_dir`（缺省开启）轮询凭证目录，
///	  指纹变化即热加载。见 `internal/pool/watch.go`。
///	· **模式 / 放行名单** —— 仍需要重启：它们是网关**启动时**读的配置，
///	  不在热加载范围内。
///
/// 故这个入口仍然必要（它同时处理上面第二类），但"加个账号也要重启"
/// 那个更烦人的症状已经消失。
///
/// 这里把三件事合成一步：
