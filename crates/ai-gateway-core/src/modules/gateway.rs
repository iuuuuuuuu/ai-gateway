...
/// 把端口规范化成网关可用的监听地址（`7863` → `127.0.0.1:7863`）。
///
/// 只绑回环是刻意的：网关默认 `api_key` 为空（= 不鉴权，见 go-gateway 的
/// `handler.authorized`），若绑到全网卡，同一局域网内任何设备都能直接调用
/// `/v1/chat/completions` 把本机账号的付费额度烧掉，`/status` 也会暴露账号
/// uid 列表。而宿主与应用本身只在 127.0.0.1 上探活与接入，绑回环零功能回归。
///
/// 确需对外暴露（Docker、反向代理）时，用户在 `gateway_native_config.json`
/// 里手写 `"listen": "0.0.0.0:7863"` 即可 —— 该文件本就是「需手动编辑」的
/// 那一层，UI 只暴露端口号。
