...
fn linux_cmdline_is_codebuddy_cn(cmdline: &str) -> bool {
    ...
    if lower.contains("ai-gateway") || lower.contains("ai-gateway") {
        return false;
    }
...
mod tests {
    ...
    fn cn_image_name_is_exact_not_international_codebuddy() {
        ...
        assert!(!is_codebuddy_cn_image_name("ai-gateway.exe"));
        assert!(!is_codebuddy_cn_image_name("ai-gateway"));
...
";
    ...
    fn linux_cmdline_matcher_accepts_cn_not_switcher() {
        ...
        assert!(!linux_cmdline_is_codebuddy_cn("/usr/bin/ai-gateway"));
    ...
    fn macos_cn_ps_filter_excludes_self_and_international() {
        ...
        let stdout = format!(
            "{self_pid} /Applications/ai-gateway.app/Contents/MacOS/ai-gateway\n\
             ...
             6003 /bin/zsh -c 'echo CodeBuddy CN.app mention via ai-gateway'\n\
