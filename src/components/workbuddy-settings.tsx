...
import { LOG_LEVEL_CLASS, LOG_TONE_CLASS, LogTail, LogTailHeader, logLevelOf } from "@/components/log-tail";
...
// workbuddy-settings.tsx —— **WorkBuddy 平台专属配置**。
//
// # 为什么在这里（2026-09-22 所有者要求）
//
// 所有者原话：
//
...
//	  设置页面只保留除了接入平台的 其他配置」
//
// 于是这四张卡片从 `SettingsPage.tsx` **原样搬来这里**（组件体一字未改），
// 再由 `AccountsPage.tsx`（WorkBuddy 账号页）渲染。
//
// 搬来的四张：
//   · 自动签到（AutoCheckinCard）—— 操作 WorkBuddy 认证文件
//   · 自动养号任务（AutoCareTasksCard）—— WorkBuddy 的 growtask 配置
//   · CodeBuddy CLI 自动轮换（AutoRotateCard）—— 轮换 WorkBuddy 账号
//   · 权限检测（PermissionCheckCard）—— 检测 WorkBuddy 认证目录可写性
//
// 它们的共同点：**全部只对 WorkBuddy 有效**，放在通用设置页会让
// Qoder/ZCode 用户看到一堆与自己无关的开关。
//
// ⚠ 组件内容与搬迁前**逐字相同** —— 这次改动只换位置，
// 不顺手改文案或行为（那会让"迁移动了逻辑吗"变得无法验证）。
 ...
 */
export function AutoCareTasksCard() {
...
export function AutoRotateCard() {
