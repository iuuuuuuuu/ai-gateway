...
3. 设置 **API Key**（留空表示不鉴权）
   - 网关**默认只绑定 `127.0.0.1`**，只有本机能访问，因此留空是安全的默认值
   - 确需让局域网/公网访问时，请**务必同时设置 API Key**：在
     `gateway_native_config.json` 里把 `listen` 改成 `0.0.0.0:7863` 并填上密钥，
     否则同网段的任何设备都能直接调用你的接口、烧掉账号额度，并通过 `/status`
     看到账号 uid 列表
...
| `api_key` | 空 | 接口鉴权密钥；空 = 不鉴权（默认只绑回环，故留空安全） |
