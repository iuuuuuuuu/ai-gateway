...
```
workbuddy-switch  (changexbc)     账号管理、桌面 GUI
        +
workbuddy2api     (Sliverkiss)    OpenAI 兼容网关
        │
        ▼  整合为单一桌面应用
ai-gateway        (momo0410)      网关页面、账号同步、单文件内嵌、分层选号
        │
        ▼  继续开发
本仓库                             代理范围可配、账号池表格化、用量口径修正、数据一致性修复
```

本仓库是**最后一层的衍生分支**：以 `momo0410/ai-gateway`（整合版）为起点，
在其上继续开发。前三层的工作都完整保留 —— 它们才是这个项目的主体，
本仓库只是接着往下做。

### 为什么要单独开一个分支

整合版是一个很好的起点，但用起来之后陆续发现几类问题，而它们的性质各不相同：

| 类别 | 例子 | 为什么值得单独做 |
|---|---|---|
| **功能缺失** | 代理只能整体开关，无法区分 Github / 国内版 / 国际版；账号池看不出「正在用哪个账号」 | 属新增能力，改动面大 |
| **口径不对** | 点账号筛选用量时，左侧数字仍是全局值 —— 看起来「点了没反应」 | 需要改数据口径，不只是改样式 |
| **数据一致性** | 积分记录出现「-380 然后 +380」的幻影配对；同一个账号在账号库里存了两条，导致禁用后状态不显示、也无法启用 | 会静默损坏用户数据，优先级最高 |
| **策略分歧** | 思考档位不被支持时，整合版选择**静默降级**（悄悄改写档位） | 见下 |

最后一条是**有意的行为分歧**，也是本仓库单独存在的主要理由之一：

> 整合版对不被支持的思考档位做静默降级（改写为最接近的支持档，只在日志留一行）。
> 本仓库改为**直接拒绝**并回以 `400 unsupported_reasoning_effort`，同时列出该模型
> 支持哪些档。
>
> 理由是：档位是客户端**显式**指定的。悄悄改写会让用户看到「我明明调了 max，
> 回答却很短」——既不知道发生了什么，也不知道该改成哪个档，只能反复试。
> 两种做法各有取舍：静默降级对「客户端从别处抄来一个档位名」更宽容，
> 直接拒绝则让问题立刻可见。**本仓库选了后者**。

### 与上游的关系

- 本仓库与 `momo0410/ai-gateway` **各自独立演进**，互不隶属，也不代表对方立场
- 上游的更新**不会**被自动合入本仓库；本仓库的改动**未必**会回流上游
- 两份都是 MIT 许可，都可以自由使用、修改与再分发 —— 用哪一份请自行判断
- 版权与署名依 MIT 要求完整保留，详见 [上游来源与许可证](#上游来源与许可证)

### 与原仓库的功能对比

下表逐项列出本仓库相对原仓库（`momo0410/ai-gateway`）的**增量**。
每一行都用脚本比对两侧源码核实过（判据取「只有实现了该功能才会出现的标识符」），
不是凭印象写的。

底子完全一致的部分（账号登录/切换/保活、到期日分层选号、图片能力下发、
成长任务一键完成、Trae 与豆包账号管理、OpenAI / Anthropic / Responses 三种协议、
熔断冷却与会话粘性等）**不在此列出** —— 那些是上游的工作，本仓库原样保留。

| 能力 | 本仓库 | 原仓库 | 说明 |
|---|:---:|:---:|---|
| **代理范围三开关** | ✅ | — | 代理地址只填一次，三个开关分别控制它作用于 Github / 国内版 / 国际版账号。默认「Github 与国际版开、国内版关」（国服直连即通，绕代理只多一跳延迟） |
| **账号池表格化** | ✅ | — | 一账号一行、列对齐便于竖着比较；点行展开二级详情，可同时展开多个；支持表头排序与全部展开 |
| **「正在使用」可见** | ✅ | — | 状态列显示「使用中 N」（N = 在途请求数）并给整行加左色条；空闲账号显示「N 分钟前用过」。不展开就能看出流量落在哪个账号 |
| **两类冷却分开显示** | ✅ | ✅ | 两边都有。本仓库补了「未知冷却类型」的兜底：不去重会拼出「冷却中 · 冷却中 42 秒」 |
| **用量互筛换口径** | ✅ | — | 点账号/模型筛选时，左侧数字**随筛选对象变化**（原口径下点了没反应，看起来像坏了） |
| **模型限制多选** | ✅ | — | 「限制使用的模型」改为多选白名单，三个工作模式都可用，默认全部放行 |
| **账号禁用 + 手动勾选模式** | ✅ | — | 支持把账号标记为禁用（不进池），工作模式合并为「自动 / 手动（勾选账号）」 |
| **活跃上报任务** | ✅ | — | 点亮连登天数（签到只恢复余额，连登天数必须靠本任务）并解锁领养前置 |
| **夜猫子任务** | ✅ | — | 23:00–08:00 时段内补一次任务，点亮仅在夜间计入的成长任务 |
| **开学季活动任务** | ✅ | — | 领取活动里已达标的奖励（只领已达标项，不伪造学生认证 / 邀请等动作） |
| **trial 加油包** | ✅ | — | 为国际版账号领取 trial 加油包（9 / 21 点，幂等自动重试） |
| **活跃地图** | ✅ | — | 补签卡保连登 + 连登档位兑换抽奖 + 礼包 / 补偿领取 |
| **自定义系统提示词** | ✅ | — | 用自带提示词文件替换客户端发出的 system 消息（关闭时原样透传） |
| **模型名前缀 `cn:` / `global:`** | ✅ | — | 按区域选号；前缀只是给网关的指令，出站前会从请求体剥离（否则上游报 11102） |
| **思考档位下发** | ✅ | — | `/v1/models` 下发各模型支持的思考档与默认档（多种字段拼写，兼容不同客户端） |
| **思考档位拒绝** | ✅ | — | 指定了不被支持的档位时**直接拒绝**并列出可用档，而非静默降级（见下方说明） |
| **模型能力下发** | ✅ | ✅ | 两边都有（含按区域维护能力真值）。本仓库在其上补了**思考档位**两个维度 |
| **积分幻影记录修复** | ✅ | — | 修复「-380 然后 +380」的幻影配对（积分包快照与可信度判定） |
| **账号库重复条目去重** | ✅ | — | 修复同一账号存两条导致「禁用了但状态不显示、也无法启用」 |
| **下拉认备注** | ✅ | — | 账号下拉与用量列表的取名口径为「备注 → 昵称 → uid 前缀」（原为「昵称 → uid 前缀」，于是填了备注等于白填） |
| **单账号记录视图** | ✅ | — | 汇总任务 / 积分 / Token 三类事件并支持日期筛选 |
| **养号任务可见可触发** | ✅ | — | 4 个任务在设置页可见、可配置时点、可「立即执行」，并把本轮结果留在卡片上 |

> 关于**思考档位**这一行：原仓库对不被支持的档位做**静默降级**（改写为最接近的
> 支持档，只在日志留一行），本仓库改为**直接拒绝**并回以
> `400 unsupported_reasoning_effort`，同时列出该模型支持哪些档。
> 两种做法各有取舍：静默降级对「客户端从别处抄来一个档位名」更宽容，
> 直接拒绝则让「调了 max 却没生效」这类问题立刻可见。**这是有意的行为分歧。**

**本仓库是否会落后于上游**：不会自动同步。若上游后续有更新而你希望合并，
可以自行 `git merge upstream/main`；本仓库的改动都在独立提交里，冲突面有限。

### 应用内更新指向本仓库

本仓库发布的安装包，其自动更新源指向**本仓库**（而非上游）。
这是刻意的：若指向上游，用户点「检查更新」会被更新成上游版本，
本仓库新增的功能会全部消失（且是不可逆的降级）。

如果你从上游安装、希望改用本仓库的版本，直接下载本仓库的安装包覆盖安装即可
（两者使用同一个应用标识与数据目录，账号与配置都会保留）。
...
> **来源声明**：本项目是**整合改造**而非从零开发。绝大多数代码来自上述上游项目；
...
把上表中零散的账号管理能力，按**你能在 WorkBuddy 里看到的那些功能**重新列一遍 ——
下面每一行都对应一个真实的客户端功能点，以及本项目让它「自动化」到了什么程度。

| WorkBuddy 功能 | 本项目怎么支持 | 自动化 |
|---|---|---|
| **登录** | OAuth 设备码扫码登录；或从本机已登录的客户端导入凭证；或手工粘贴 Token | 手动触发 |
| **凭证保活** | 惰性刷新（操作前低于阈值即刷新）+ 每日全量保活，避免 refresh token 过期失效 | ✅ 自动 |
| **每日签到** | 启动即核验，运行期周期性补签；保留 30 天签到日志 | ✅ 自动 |
| **积分（余额）查询** | 查询各账号积分资源、剩余量与到期时间；7 天内到期高亮并优先排序 | ✅ 自动巡检 |
| **连登天数** | 由「活跃上报」任务点亮（**签到只恢复余额，连登天数必须靠本任务**） | ✅ 自动 |
| **成长任务** | 活跃上报（对话事件、进入 Buddy 应用等）、夜猫子任务（23:00–08:00 专属） | ✅ 按时点自动 |
| **积分到期不浪费** | 按积分到期日分层选号，先烧快过期的额度；同一天到期的账号平均分摊 | ✅ 自动 |
| **开学季活动** | 领取活动里已达标的奖励（只领已达标项，不伪造学生认证 / 邀请等动作） | ✅ 按时点自动 |
| **trial 加油包** | 为国际版账号领取 trial 加油包 | ✅ 按时点自动 |
| **猫猫旅行** | 随签到时点自动巡检并派发 / 领取 | ✅ 自动 |
| **账号切换** | WorkBuddy 客户端、CodeBuddy CLI、CodeBuddy CN IDE 三套登录态独立切换 | 手动触发 |
| **会话迁移** | 把当前账号的会话以新 ID 复制给目标账号（含 jsonl 正文、数据库索引、edge-sync 注册） | 手动触发 |
| **官方用量统计** | 汇总官方请求用量：每日趋势、模型分布、账号消耗、请求明细 | 手动查看 |
| **把额度开放成 API** | 账号池暴露为 OpenAI 兼容接口，供任意客户端零改造接入 | ✅ 常驻 |

**「养号任务」的完整动作清单**（`go-gateway/internal/growtask/actions.go`）——
除了上面几项，网关还会按需执行这些，用于点亮成长任务：

| 动作 | 说明 |
|---|---|
| `chat_5` | 补报对话活跃事件至达标（按差额） |
| `first_buddy` | 上报解锁 → 同意协议 → 领养第一只 Buddy |
| `RichMeow_Chat` | 上报桌面端完整对话事件链（含成功回执） |
| `Buddy_App` / `Buddy_App_QQ` | 上报进入 Buddy 应用 / 企鹅教师助手事件链 |
| `automation_1` | 上报定时任务创建成功事件 |
| `Library_read` | 以 Web 指纹上报资料库阅读事件 |
| `template_5` | 上报模板使用事件组 ×5 |
| `playbook_prompt` | 上报灵感案例「做同款」发送事件组 |
| `create_canvas` | 上报设计创意画布创建事件组 |
| `Hp_Appearance` | 设置主题并上报皮肤生效事件 |
| `Model_chat_GLM5.2` | glm-5.2 真实对话一次并对齐模型上报 |
| `expert_5` / `Expert_team_use_3` / `Expert_lighthouse` | 真实专家 / 专家团 / 轻量云专家召唤 + 使用链 |
| `skill_1` | 真实对话 + 技能加载（`skill_info`）事件 |
| `black_cat` | 夜猫时段内补足真实对话（23:00–08:00 北京时间） |

> 标 `NeedsChat: true` 的动作会**真的发一条对话**（走 chat 域），其余走活跃上报
> （billing 域）。因此一轮活跃上报的耗时随账号数线性增长（实测 19 个账号约 41.7 秒）。
...
| **代理范围可配** | 代理地址只填一次，**三个开关**分别控制它作用于 Github（检查更新/下载）、国内版账号、国际版账号。默认按「升级前后行为不变」取值：Github 与国际版开、国内版关（国服直连即通，绕进代理只会多一跳延迟与一个故障面） |
| **账号池表格化** | 一账号一行、列对齐便于竖着比较；点行展开二级详情（模型冷却列表 / 运行态 / 积分），可同时展开多个账号；支持表头排序与全部展开 |
| **「正在使用」可见** | 状态列显示「使用中 N」（N = 该账号此刻的在途请求数）并给整行加左色条；空闲账号显示「N 分钟前用过」。不展开就能看出流量落在哪个账号上 |
| **两类冷却分开显示** | 账号级冷却（整号不可用）与模型级冷却（仅这几个模型不可用、换模型仍可用）在界面上分开表达，后者用角标计数 + 悬浮明细 |
| **思考档位下发与校验** | `/v1/models` 下发各模型支持的思考档与默认档（多种字段拼写，兼容不同客户端）；客户端指定了不被支持的档位时**直接拒绝**并列出可用档，而不是静默改写 |
| **模型能力下发** | `/v1/models` 下发图片能力（多种拼写），修复「有多模态模型却发不出图片」 |
...
> Windows x64 上实测 `cargo test --workspace` 全部通过（666 个用例，0 失败）。
...
`workbuddy2api`（Go 网关）的源码**已直接合入 `go-gateway/`**，随仓库一并分发，
无需另行 clone 上游或打补丁。下列改动叠加在 `workbuddy2api` 之上
（最初的补丁基于上游 `cfb1713` 生成）。

> 与整合版的差异：**思考档位的处理策略不同**。整合版对不被支持的档位做**静默降级**
> （改写为最接近的支持档，只在日志留一行）；本仓库改为**直接拒绝**并回以
> `400 unsupported_reasoning_effort`，同时列出该模型支持哪些档。
> 理由：档位是客户端显式指定的，悄悄改写会让用户看到「调了 max 却答得很短」，
> 既不知道发生了什么，也不知道该改成哪个档。两种策略各有取舍 ——
> 静默降级对「客户端从别处抄来一个档位名」更宽容，直接拒绝则让问题立刻可见。
...
| [ai-gateway](https://github.com/momo0410/ai-gateway) | [momo0410](https://github.com/momo0410) | 把上面两者整合为单一桌面应用的工作（网关页面、账号同步、单文件内嵌、单实例保护、按到期日分层选号等） | **MIT** |

三者均采用 **MIT 许可证**，允许使用、修改与再分发。本仓库已保留其原始版权声明
（见 [`LICENSE`](./LICENSE)）。

> 本仓库是**衍生分支**：在上表中第三个项目（整合版）的基础上继续开发，
> 与其**相互独立、各自演进**。整合版的后续更新不会被自动合入本仓库；
> 本仓库的改动也未必会回流。本项目不代表上述任何作者的立场或背书。

本仓库新增部分同样以 MIT 许可证发布。逐项来源说明与改动清单见
[`NOTICE`](./NOTICE)。
...
Copyright (c) 2026 changexbc         （workbuddy-switch 原作者）
Copyright (c) 2026 Sliverkiss        （workbuddy2api 原作者）
Copyright (c) 2026 momo0410          （ai-gateway 整合部分）
```
...
本项目的绝大部分代码来自两个上游开源项目，作者分别是
[changexbc](https://github.com/changexbc) 与 [Sliverkiss](https://github.com/Sliverkiss)；
桌面整合与网关增强部分由 [momo0410](https://github.com/momo0410) 完成。
若这些工作对你有帮助，欢迎到上面几个仓库点一个 Star。

本仓库是上述工作的**衍生分支**，不代表上游作者的立场，也不由上游作者维护。
