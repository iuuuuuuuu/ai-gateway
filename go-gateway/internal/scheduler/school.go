...
func (s *Scheduler) runSchool(ctx context.Context) {
	...
	for _, st := range s.cfg.Pool.List() {
		...
		claimed := s.claimFinishedTasks(a, tasks)
		if claimed > 0 {
...
// claimFinishedTasks 领取已达标的任务奖励，返回成功领取的数量。
//
// 只处理 status == "finished"（已达标待领取）的任务；
// pending（未达标）与 claimed（已领取）都跳过 —— 前者没资格、后者会报重复领取。
func (s *Scheduler) claimFinishedTasks(a *auth.Auth, tasks []upstream.SchoolTask) int {
	n := 0
	for _, t := range tasks {
