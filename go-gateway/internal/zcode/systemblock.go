...
//
// # 2026-09-20 晚：3012 自行消失（所有者确认「zcode也可以进行对话了」）
//
// 这**证实**了上面的结论，且补上了它缺的那一半：
// 3012 不是永久封禁，而是**一段有时间尺度的风控** —— 会自己过期。
// 故本模块的立场不变：**它不是 3012 的答案，默认关闭**。
//
// ⚠ 反过来说也别把 3012 当"功能没做完"：端点、请求形状、deviceMid、
// param 复用、system 块 —— 这些方向**全部被证伪**。判据只有一条：
// **官方客户端是否也吃 3012**（它吃，就不是我们的问题）。
//
