...
//
// 根因：readLimitedBody 用 io.LimitReader(8MB)，读满即返回，调用方无法区分
// 「读完了」与「被截断了」。截断体不是合法 JSON，prepareBody 解析失败后原样透传，
// 上游 json.Decoder 报 unexpected EOF。客户端只看到「请求参数有误」。
// ---------------------------------------------------------------------------
...
func TestOversizedBodyRejectedNotTruncated(t *testing.T) {
	...
	if rec.Code != http.StatusRequestEntityTooLarge {
		t.Fatalf("code=%d want 413；body=%s", rec.Code, truncateBodyForLog(rec.Body.String()))
	}
	var e struct {
		Error struct {
...
// errBodyTooLarge 哨兵，对正常体原样返回，空体保持原有语义。
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			if !strings.Contains(rec.Body.String(), c.wantCode) {
				t.Errorf("body 应含错误码 %q，实际=%s", c.wantCode, truncateBodyForLog(rec.Body.String()))
			}
		})
	}
}

// truncateBodyForLog 截断过长的响应体用于失败信息（避免刷屏）。
func truncateBodyForLog(s string) string {
	if len(s) > 300 {
		return s[:300] + "..."
	}
	return s
