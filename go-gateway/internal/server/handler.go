...
type Config struct {
	...
	//
	// 为什么每个产品一个字段而不是一张 map：
	//   · 产品数量是**固定的少数几个**（WorkBuddy 走默认路径，其余各一个）；
	//   · 字段能让"哪些产品已接入"在 Config 上一眼看清，而 map 要读代码才知道；
	//   · 编译期类型安全（map 要 interface{} 或统一接口，丢失具体类型）。
	Qoder ProductUpstream

	// Zcode ZCode（Z.AI / 智谱）产品的上游实现；nil = 该产品不可用。
	Zcode ProductUpstream

	// ProductModels 各产品**实际可用**的模型清单（由宿主透传）。
	//
	// 形状：`{"qoder":["Qwen3.8-Max"],"zcode":["glm-5.3"]}`
	//
	// 用途：`/v1/models` 的 `channels` 字段 —— 告诉界面"这个模型来自
	// 哪个平台"。同一个 id 出现在多个产品里时就有多个渠道，即使用者要的
	// 「重叠就显示多个平台」。
	//
	// # 为什么由宿主透传而不是网关自己去查
	//
	// `/v1/models` 在**请求路径**上：每个客户端列一次模型就打一次上游，
	// 会带来延迟与失败面。而"某产品的账号能用哪些模型"只有宿主知道
	//（它持有账号库，刷新账号时已查过并按 uid 缓存）。
	//
	// 与 `RunTaskFor` / `GrowthTasks` 同样是**依赖倒置**：网关只消费
	// 宿主已经算好的结果，不反向依赖宿主的模块。
	//
	// nil / 空 = 只有 WorkBuddy 的模型带渠道（降级但不报错）。
