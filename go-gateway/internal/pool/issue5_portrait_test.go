...
// 但它们的 until 是零值、cool_kind 是历史残留的 hard_credit。旧实现按
...
// TestStatusBreakerReportsEffectiveDeadline 熔断生效时，剩余秒数按两个截止中
// **较晚**者算，类型报 "breaker"（而不是残留的 hard_credit）。
func TestStatusBreakerReportsEffectiveDeadline(t *testing.T) {
	...
	// healthy() 要求 until 与 breakerUntil 都过期 → 恢复时刻是较晚的熔断截止（约 2h）。
...
func TestStatusSoftCooldownStillReportsItsOwnKind(t *testing.T) {
	p := New("")
	p.Add(&auth.Auth{UID: "u1"})
	p.SetBreaker(3, time.Hour, time.Hour) // 阈值 3：单次冷却不熔断
